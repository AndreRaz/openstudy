...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall openstudy and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenStudy")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      ...
      brew: "brew uninstall openstudy",
      choco: "choco uninstall openstudy",
      scoop: "scoop uninstall openstudy",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "openstudy", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
